...
describe("getUsersApi", () => {
  it("should return users data correctly when API is called", async () => {
  ...
  it("should return error message when API is failed", async () => {
    nock("https://dummyjson.com")
      .get("/users")
      .reply(500, "Internal Server Error");

    const result = await getUsersApi();
    expect(result._unsafeUnwrapErr()).toEqual("failed to get users");
  });
});
